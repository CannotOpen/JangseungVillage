...
/// https://gist.github.com/Curookie/1f978c64b9a2457dab64281299934c40
/// </summary>
...
4) PlayerPrefabs으로 설정을 저장하며 토글 속성 있다. ex)IsMusicOn 배경음, IsSoundOn 효과음";
    ...
    void Initialize()
    {
        ...
        musicOn = _musicOn = LoadBGMMuteStatus();
        currentMusicVol = _musicVolume = _useMusicVolOnStart ? _musicVolume : LoadBGMVolume();
        sfxOn = _soundFxOn = LoadSFXMuteStatus();
        currentSfxVol = _soundFxVolume = _useSfxVolOnStart ? _soundFxVolume : LoadSFXVolume();
    ...
    public AudioSource PlaySFX(SoundName clip, bool singleton = false, Action callback = null)
    {
        return PlaySFX(clip, singleton, callback);
    }
    ...
    public AudioSource PlayOneShot(SoundName clip, Action callback = null)
    {
        return PlayOneShot(clip.ToString(), callback);
    }
    ...
    {
        using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip(audio_url, audio_type))
        {
